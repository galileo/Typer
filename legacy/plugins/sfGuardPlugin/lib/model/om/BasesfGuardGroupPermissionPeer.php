...
abstract class BasesfGuardGroupPermissionPeer {
		...
		while($rs->next()) {
			...
			foreach($results as $temp_obj1) {
				...
				if ($temp_obj2->getPrimaryKey() === $obj2->getPrimaryKey()) {
					...
					$temp_obj2->addsfGuardGroupPermission($obj1); //CHECKME
					break;
				}
			}
			if ($newObject) {
				$obj2->initsfGuardGroupPermissions();
				$obj2->addsfGuardGroupPermission($obj1); //CHECKME
			}
			$results[] = $obj1;
		}
		return $results;
	...
	public static function doSelectJoinsfGuardPermission(Criteria $c, $con = null)
	{
		...
		while($rs->next()) {
			...
			$newObject = true;
			foreach($results as $temp_obj1) {
				$temp_obj2 = $temp_obj1->getsfGuardPermission(); //CHECKME
				if ($temp_obj2->getPrimaryKey() === $obj2->getPrimaryKey()) {
					$newObject = false;
					// e.g. $author->addBookRelatedByBookId()
					$temp_obj2->addsfGuardGroupPermission($obj1); //CHECKME
	...
	public static function doSelectJoinAll(Criteria $c, $con = null)
	{
		...
		while($rs->next()) {
			...
			for ($j=0, $resCount=count($results); $j < $resCount; $j++) {
				$temp_obj1 = $results[$j];
				$temp_obj3 = $temp_obj1->getsfGuardPermission(); // CHECKME
				if ($temp_obj3->getPrimaryKey() === $obj3->getPrimaryKey()) {
					$newObject = false;
					$temp_obj3->addsfGuardGroupPermission($obj1); // CHECKME
					break;
				}
			}
